...
async def _probe(ip: str) -> dict | None:
    ...
    # Открытого порта мало. Если сеть управления пропала, соединение уходит
    # через шлюз по умолчанию, и тот принимает его на любой адрес: в списке
    # оказывается вся подсеть. Настоящий сервер SSH по RFC 4253 представляется
    # первым, и молчание в ответ означает, что это не он.
